...
```{r, echo = TRUE}
G5_ssmv <- read_csv("G5_ssmv-inst-transcripts.csv")
```
...
```{r, eval=F}
write_csv(G5_ssmv, "data/G5_ssmv_copy.csv")
```
...
```{r}
G5_ssmv_renamed <- rename(G5_ssmv, transcript = Talk)
```
...
```{r, echo=F}
G5_ssmv_tidy <- unnest_tokens(G5_ssmv_renamed, word, transcript)
...
G5_ssmv_clean <- anti_join(G5_ssmv_tidy, stop_words)
...
```{r}
G5_ssmv_resource_counts <- G5_ssmv_clean %>%
  count(ID, word, sort = TRUE)

view(G5_ssmv_resource_counts)
```
...
```{r}
G5_ssmv_frequencies <- G5_ssmv_clean %>%
  count(ID, word, sort = TRUE) %>%
  group_by(ID) %>%
